...
## Premium invitation links (R28)

Guest invites send a short `…/i/<id>` link. WhatsApp shows a rich gold
preview generated by `app/i/[token]/opengraph-image.tsx` (Heebo font
bundled in `assets/`). Requires the `2026-05-17-short-links.sql`
migration; without it the flow falls back to the full URL.

### How to refresh a WhatsApp preview cache

WhatsApp/Facebook cache the OG image per URL. If you changed the design
or the event details after a link was already shared, force a re-scrape:

1. Open the **Facebook Sharing Debugger**:
   <https://developers.facebook.com/tools/debug/sharing/>
2. Paste the full `https://<site>/i/<id>` link.
3. Click **Scrape Again** (do it twice — the first pull warms the cache,
   the second serves the fresh image).
4. New WhatsApp shares of that link now show the updated preview.
   (Already-sent messages keep the old cached image — only new sends
   pick up the refresh.)
